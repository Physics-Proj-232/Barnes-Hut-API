...
#include "BH_physics_operation.h"
#include "checker.h"
#include <stdlib.h>
#include <stdio.h>

void createChildren(Float4 *node)
{
    int i = 0;
    while (i < 8)
    {
        node children = new Float4;
...
t_octant *octree_devide(t_octant *root)
{
    int i = 0;
    size_t end root->end / 8;
    size_t start = 0;
    createChildren(root);
    while (i < 8)
    {
    ...
    while (i > 0)
    {
        octree_devide(root->children[i-1]);
        i--;
    }
    return (root);
}
